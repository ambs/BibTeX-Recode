...
sub latex_decode {
    ...
    my $norm_form
        = exists $opts{normalization} ? $opts{normalization} : 'NFD';
    ...
    foreach my $type (
        'greek',           'dings',
        'punctuation',     'symbols',
        'negatedsymbols',  'superscripts',
        'cmdsuperscripts', 'letters',
        'diacritics'
        )
    {
...
sub latex_encode {
    ...
    foreach my $type (
        qw'greek dings negatedsymbols superscripts cmdsuperscripts
           diacritics letters punctuation symbols'
        )
    {
        ...
        if ( $type eq 'negatedsymbols' ) {
            ...
            $text
                =~ s/($re)/($remap_e_raw->{$1} ? '' : "\\") . $map->{$1} . ($remap_e_raw->{$1} ? '' : '{}')/ge;
        }
        elsif ( first { $type eq $_ } ( 'punctuation', 'symbols', 'greek' ) )
        {
